...
public class MinimumDRStrategy {
	private MinimumDRDatumDataSource settings;
	...
	public MinimumDRDatumDataSource getSettings() {
		return settings;
	}

	// configured in OSGI
	public void setSettings(MinimumDRDatumDataSource settings) {
				...
				if (DRSupportTools.isDRCapable(params)) {
					...
					if (DRSupportTools.isChargeable(params)) {
						...
						if (mode.equalsIgnoreCase("DISCHARGE")) {
							...
							// Tell the battery to charge as much as it can
							sendDRtoBattery(false, handler, maxwatts);
						} else {
